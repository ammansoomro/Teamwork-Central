...
void Footways::intersectWithBuilding(Building &B) {
  ...
  //
  sort(footwayIDs.begin(), footwayIDs.end(), [](long long id1, long long id2) {
    if (id1 < id2)
      return true;
    else
      return false;
...
void Footways::addEdgesToGraph(graph &G, Nodes &N) {
  int count = 1;
  for (int i = 0; i < this->getNumMapFootways(); i++) {
    for (int j = 0; j < (int)this->MapFootways[i].NodeIDs.size() - 1; j++) {
